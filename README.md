...
`digides-ogimaker` (Digital Desa OG Image Maker), a cloudflare worker to generate OpenGraph banner, configured specifically for Digital Desa Profile Website.
...
This project is built on top of WASIP1 instead of javascript WASM. The flow of data is greatly simplified, reading data from `stdin` and write to `stdout`. Since there is no necessity for memory reading yet, the WASI code is pretty simple.
